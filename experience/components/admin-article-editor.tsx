...
type LibraryVideo = { id: string; title: string; platform: string; video_id: string | null; video_url: string; thumbnail: string | null; is_published: boolean }
type GalleryPick = { id: string; title: string; alt_text: string; media_type: 'image' | 'video'; media_url: string }
...
export function AdminArticleEditor({client,user,id}:{client:Db;user:{id:string};id:'new'|string}){
 ...
 const [libraryVideos,setLibraryVideos]=useState<LibraryVideo[]>([]),[galleryItems,setGalleryItems]=useState<GalleryPick[]>([]),[videoQuery,setVideoQuery]=useState(''),[galleryQuery,setGalleryQuery]=useState('')
 const set=<K extends keyof Article>(key:K,value:Article[K])=>setData(v=>({...v,[key]:value}))
 const appendContent=(html:string)=>{if(!html)return;setData(v=>({...v,content:v.content?`${v.content}\n${html}`:html}));setNotice('Media inserito nel testo.')}

 const load=useCallback(async()=>{
  setBusy(true)
  try{
   const [cats,people,others,videosRes,galleryRes]=await Promise.all([
    ...
    client.from('videos').select('id,title,platform,video_id,video_url,thumbnail,is_published').eq('is_published',true).order('updated_at',{ascending:false}).limit(80),
    client.from('gallery_items').select('id,title,alt_text,media_type,media_url').eq('status','published').order('captured_at',{ascending:false}).limit(60),
   ])
   ...
   setLibraryVideos((videosRes.data||[]) as LibraryVideo[])
   setGalleryItems((galleryRes.data||[]) as GalleryPick[])
   if(!isNew){
 ...
 const filteredVideos=useMemo(()=>{const q=videoQuery.trim().toLowerCase();if(!q)return libraryVideos;return libraryVideos.filter(v=>v.title.toLowerCase().includes(q))},[libraryVideos,videoQuery])
 const filteredGallery=useMemo(()=>{const q=galleryQuery.trim().toLowerCase();if(!q)return galleryItems;return galleryItems.filter(g=>`${g.title} ${g.alt_text}`.toLowerCase().includes(q))},[galleryItems,galleryQuery])
 ...
 async function save(){setBusy(true);try{
  const previousStatus=isNew?null:(await client.from('articles').select('status').eq('id',id).maybeSingle()).data?.status||null
  const articleId=await persist()
  const slug=data.slug||slugify(data.title)
  const becamePublished=data.status==='published'&&previousStatus!=='published'
  if(becamePublished){
   try{
    const {data:session}=await client.auth.getSession()
    const token=session.session?.access_token
    if(token){
     const site=(typeof window!=='undefined'?window.location.origin:'https://bianconerihub.com')
     await fetch('/api/telegram/announce',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({title:data.title,excerpt:data.excerpt,url:`${site}/articolo/${slug}`,cover_image:data.cover_image||undefined,articleId})})
    }
   }catch(e){console.error('[telegram-announce]',e)}
  }
  setNotice(becamePublished?'Articolo pubblicato (canale Telegram avvisato se configurato).':'Articolo salvato.')
  if(isNew)router.replace(`/admin/articoli/${articleId}/modifica`);else await load()
 }catch(e){setNotice(err(e))}finally{setBusy(false)}}
 ...
 return <div className="admin-article-editor grid gap-5 text-[#f4f2ec]">
  ...
  <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><main className="grid content-start gap-5">
   ...
   <section className="grid gap-3 rounded-xl border border-[#303130] bg-[#101111] p-5"><h3 className="font-serif text-xl">Media</h3><Input value={data.cover_image} placeholder="URL copertina" onChange={e=>set('cover_image',e.target.value)}/><Label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-[#555] p-3"><Upload/>Carica copertina<Input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={e=>e.target.files?.[0]&&upload(e.target.files[0],'cover_image')}/></Label><div className="flex gap-2"><Input value={galleryUrl} placeholder="URL galleria" onChange={e=>setGalleryUrl(e.target.value)}/><Button type="button" size="icon" variant="outline" aria-label="Aggiungi alla galleria" onClick={()=>{if(galleryUrl.trim()){set('gallery',[...data.gallery,galleryUrl.trim()]);setGalleryUrl('')}}}><ImagePlus/></Button></div><Label className="flex cursor-pointer items-center gap-2 rounded-lg border border-dashed border-[#555] p-3"><Upload/>Carica in galleria<Input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={e=>e.target.files?.[0]&&upload(e.target.files[0],'gallery')}/></Label>{data.gallery.map((url,i)=><div className="flex gap-2" key={`${url}-${i}`}><Input value={url} readOnly/><Button type="button" variant="destructive" onClick={()=>set('gallery',data.gallery.filter((_,n)=>n!==i))}>Rimuovi</Button></div>)}
    <div className="mt-2 grid gap-2 border-t border-[#303130] pt-3"><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-medium">Videoteca</h4><a className="text-xs text-[#bd9b61] underline" href="/admin/video">Gestisci video</a></div><Input value={videoQuery} placeholder="Cerca video…" onChange={e=>setVideoQuery(e.target.value)}/><div className="max-h-40 space-y-2 overflow-auto rounded-lg border border-[#383938] p-2">{filteredVideos.length?filteredVideos.map(v=><div key={v.id} className="flex items-center justify-between gap-2 text-sm"><span className="min-w-0 truncate"><span className="text-[#888]">{v.platform}</span> · {v.title}</span><Button type="button" size="sm" variant="outline" onClick={()=>appendContent(videoToArticleHtml(v))}>Inserisci</Button></div>):<p className="text-xs text-[#888]">Nessun video pubblicato.</p>}</div></div>
    <div className="grid gap-2 border-t border-[#303130] pt-3"><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-medium">Gallery Live</h4><a className="text-xs text-[#bd9b61] underline" href="/admin/gallery">Apri gallery</a></div><Input value={galleryQuery} placeholder="Cerca in gallery…" onChange={e=>setGalleryQuery(e.target.value)}/><div className="max-h-48 space-y-2 overflow-auto rounded-lg border border-[#383938] p-2">{filteredGallery.length?filteredGallery.map(g=><div key={g.id} className="flex items-center gap-2 text-sm"><div className="h-10 w-14 shrink-0 overflow-hidden rounded bg-black">{g.media_type==='video'?<video src={g.media_url} muted playsInline preload="metadata" className="h-full w-full object-cover"/>:<img src={g.media_url} alt="" className="h-full w-full object-cover"/>}</div><span className="min-w-0 flex-1 truncate"><span className="text-[#888]">{g.media_type==='video'?'Video':'Foto'}</span> · {g.title}</span><Button type="button" size="sm" variant="outline" onClick={()=>appendContent(galleryItemToArticleHtml(g))}>Inserisci</Button></div>):<p className="text-xs text-[#888]">Nessun media pubblicato in gallery.</p>}</div></div>
   </section>
